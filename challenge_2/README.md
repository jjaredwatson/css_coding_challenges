...
In this exercise, we'll take a series of simple comps and recreate each in an individual folder. You will make the appropriate number of `<div>` tags with the div numbers indicated in the comps, give them a `<height>`, `<width>`, and `background-color` and then make them match the layout in the comp. Here's an example of what this might look like in your HTML and CSS:
...
When you have completed all the layout challenges and saved the css and html for each, make a `SOLUTIONS.md` file with links to all of your solutions. In your `SOLUTIONS.md` include the challenge number, and a brief explanation of the approach/thinking you used to solve each layout challenge. The structure of your `SOLUTION.md` content should be written in [Markdown](https://github.com/adam-p/markdown-here/wiki/Markdown-Cheatsheet), a simple text formatting syntax that you can use when you write a gist. Here's an example of what the structure of your gist should look like:
